...
---
# <a name="nullable-reference-type-annotations-changed"></a><span data-ttu-id="a0ebd-103">可以为 Null 的引用类型注释已更改</span><span class="sxs-lookup"><span data-stu-id="a0ebd-103">Nullable reference type annotations changed</span></span>

<span data-ttu-id="a0ebd-104">此问题表示一项正在进行的工作。在整个 ASP.NET Core 6.0 中，所有对为 Null 性注释的中断性变更都将汇总到这个问题中。</span><span class="sxs-lookup"><span data-stu-id="a0ebd-104">_**This issue represents a work-in-progress. All breaking changes to nullability annotations will be aggregated into this issue throughout the course of ASP.NET Core 6.0.**_</span></span>

<span data-ttu-id="a0ebd-105">从 ASP.NET Core 5.0 开始，为 Null 性注释已应用到部分代码中。</span><span class="sxs-lookup"><span data-stu-id="a0ebd-105">Starting in ASP.NET Core 5.0, nullability annotations have been applied to parts of the code.</span></span> <span data-ttu-id="a0ebd-106">从这项工作一开始，就预料到这些注释会[出现错误](https://github.com/dotnet/runtime/blob/master/docs/coding-guidelines/api-guidelines/nullability.md#breaking-change-guidance)，需要进行修正。</span><span class="sxs-lookup"><span data-stu-id="a0ebd-106">From the outset of this effort, [mistakes were expected](https://github.com/dotnet/runtime/blob/master/docs/coding-guidelines/api-guidelines/nullability.md#breaking-change-guidance) in these annotations and fixes would need to be made.</span></span> <span data-ttu-id="a0ebd-107">在 ASP.NET Core 6.0 中，将对某些以前应用的批注进行更新。</span><span class="sxs-lookup"><span data-stu-id="a0ebd-107">In ASP.NET Core 6.0, some previously applied annotations are being updated.</span></span> <span data-ttu-id="a0ebd-108">其中的某些更改被视为源中断性变更。</span><span class="sxs-lookup"><span data-stu-id="a0ebd-108">Some of these changes are considered source breaking changes.</span></span> <span data-ttu-id="a0ebd-109">这些更改导致 API 不兼容或具有更大的限制性。</span><span class="sxs-lookup"><span data-stu-id="a0ebd-109">The changes lead to the APIs being incompatible or more restrictive.</span></span> <span data-ttu-id="a0ebd-110">如果在启用了可以为 Null 的引用类型的项目中使用，则更新后的 API 可能会导致生成时警告。</span><span class="sxs-lookup"><span data-stu-id="a0ebd-110">The updated APIs may result in build-time warnings when used in projects that have nullable reference types enabled.</span></span>

<span data-ttu-id="a0ebd-111">有关讨论，请参阅 GitHub 问题 [dotnet/aspnetcore#27564](https://github.com/dotnet/aspnetcore/issues/27564)。</span><span class="sxs-lookup"><span data-stu-id="a0ebd-111">For discussion, see GitHub issue [dotnet/aspnetcore#27564](https://github.com/dotnet/aspnetcore/issues/27564).</span></span>

## <a name="version-introduced"></a><span data-ttu-id="a0ebd-112">引入的版本</span><span class="sxs-lookup"><span data-stu-id="a0ebd-112">Version introduced</span></span>

<span data-ttu-id="a0ebd-113">6.0</span><span class="sxs-lookup"><span data-stu-id="a0ebd-113">6.0</span></span>

## <a name="old-behavior"></a><span data-ttu-id="a0ebd-114">旧行为</span><span class="sxs-lookup"><span data-stu-id="a0ebd-114">Old behavior</span></span>

<span data-ttu-id="a0ebd-115">受影响的 API 具有不正确的可以为 Null 引用类型注释。</span><span class="sxs-lookup"><span data-stu-id="a0ebd-115">The affected APIs have incorrect nullable reference type annotations.</span></span> <span data-ttu-id="a0ebd-116">生成警告不存在或不正确。</span><span class="sxs-lookup"><span data-stu-id="a0ebd-116">Build warnings are either absent or incorrect.</span></span>

## <a name="new-behavior"></a><span data-ttu-id="a0ebd-117">新行为</span><span class="sxs-lookup"><span data-stu-id="a0ebd-117">New behavior</span></span>

<span data-ttu-id="a0ebd-118">生成新的生成警告。</span><span class="sxs-lookup"><span data-stu-id="a0ebd-118">New build warnings are produced.</span></span> <span data-ttu-id="a0ebd-119">不再为受影响的 API 生成不正确的生成警告。</span><span class="sxs-lookup"><span data-stu-id="a0ebd-119">Incorrect build warnings are no longer produced for the affected APIs.</span></span>

## <a name="reason-for-change"></a><span data-ttu-id="a0ebd-120">更改原因</span><span class="sxs-lookup"><span data-stu-id="a0ebd-120">Reason for change</span></span>

<span data-ttu-id="a0ebd-121">通过反馈和进一步测试，已确定受影响 API 的“为 Null 性”注释是不准确的。</span><span class="sxs-lookup"><span data-stu-id="a0ebd-121">Through feedback and further testing, the nullable annotations for the affected APIs were determined to be inaccurate.</span></span> <span data-ttu-id="a0ebd-122">更新后的注释现在正确地表示了 API 的“为 Null 性”协定。</span><span class="sxs-lookup"><span data-stu-id="a0ebd-122">The updated annotations now correctly represent the nullability contracts for the APIs.</span></span>

## <a name="recommended-action"></a><span data-ttu-id="a0ebd-123">建议的操作</span><span class="sxs-lookup"><span data-stu-id="a0ebd-123">Recommended action</span></span>

<span data-ttu-id="a0ebd-124">更新调用这些 API 的代码，以反映修订后的“为 Null 性”协定。</span><span class="sxs-lookup"><span data-stu-id="a0ebd-124">Update code calling these APIs to reflect the revised nullability contracts.</span></span>

## <a name="affected-apis"></a><span data-ttu-id="a0ebd-125">受影响的 API</span><span class="sxs-lookup"><span data-stu-id="a0ebd-125">Affected APIs</span></span>
...
## <a name="see-also"></a><span data-ttu-id="a0ebd-126">另请参阅</span><span class="sxs-lookup"><span data-stu-id="a0ebd-126">See also</span></span>

- [<span data-ttu-id="a0ebd-127">核心 .NET 库中可以为 null 的引用类型注释更改</span><span class="sxs-lookup"><span data-stu-id="a0ebd-127">Nullable reference type annotation changes in core .NET libraries</span></span>](../../core-libraries/6.0/nullable-ref-type-annotation-changes.md)
