...
## <a name="run-apiport-to-identify-problematic-apis"></a><span data-ttu-id="c3b67-117">运行 *ApiPort* 以识别有问题的 api</span><span class="sxs-lookup"><span data-stu-id="c3b67-117">Run *ApiPort* to identify problematic APIs</span></span>
...
<span data-ttu-id="c3b67-156">为现有 ASP.NET 项目创建新项目文件的常见方法是 `dotnet new`   >    >  在 Visual Studio 中使用或 "新建 **项目**" 创建新的 ASP.NET Core 应用。</span><span class="sxs-lookup"><span data-stu-id="c3b67-156">A common way to create a new project file for an existing ASP.NET project is to create a new ASP.NET Core app using `dotnet new` or **File** > **New** > **Project** in Visual Studio.</span></span> <span data-ttu-id="c3b67-157">然后，可以将文件从旧项目复制到新项目，以完成迁移。</span><span class="sxs-lookup"><span data-stu-id="c3b67-157">Then files can be copied from the old project to the new one to complete the migration.</span></span>
...
<span data-ttu-id="c3b67-173">下一个对话框将要求您选择要使用的模板。</span><span class="sxs-lookup"><span data-stu-id="c3b67-173">The next dialog will ask you to choose which template to use.</span></span> <span data-ttu-id="c3b67-174">选择“空”模板。</span><span class="sxs-lookup"><span data-stu-id="c3b67-174">Select the **Empty** template.</span></span> <span data-ttu-id="c3b67-175">还要确保将下拉列表从 **.Net Core** 更改为 **.NET Framework**。</span><span class="sxs-lookup"><span data-stu-id="c3b67-175">Be sure to also change the dropdown from **.NET Core** to **.NET Framework**.</span></span> <span data-ttu-id="c3b67-176">选择 **ASP.NET Core 2.2**，如图4-9 中所示。</span><span class="sxs-lookup"><span data-stu-id="c3b67-176">Select **ASP.NET Core 2.2**, as shown in Figure 4-9.</span></span>
...
<span data-ttu-id="c3b67-271">这次，有16个错误：</span><span class="sxs-lookup"><span data-stu-id="c3b67-271">This time, there are 16 errors:</span></span>
...
### <a name="configure-mvc"></a><span data-ttu-id="c3b67-322">配置 MVC</span><span class="sxs-lookup"><span data-stu-id="c3b67-322">Configure MVC</span></span>
...
<span data-ttu-id="c3b67-356">现在是时候配置 MVC 服务，接下来是应用程序对依赖关系注入的支持 (DI) 。</span><span class="sxs-lookup"><span data-stu-id="c3b67-356">Now it's time to configure MVC services, followed by the rest of the app's support for dependency injection (DI).</span></span> <span data-ttu-id="c3b67-357">到目前为止， *eShopPorted* 项目的 `ConfigureServices` 方法保持为空。</span><span class="sxs-lookup"><span data-stu-id="c3b67-357">So far, the *eShopPorted* project's `ConfigureServices` method has remained empty.</span></span> <span data-ttu-id="c3b67-358">现在是时候开始填充它了。</span><span class="sxs-lookup"><span data-stu-id="c3b67-358">Now it's time to start populating it.</span></span>
...
<span data-ttu-id="c3b67-377">目前，的设置 `useMockData` 设置为 `true` 。</span><span class="sxs-lookup"><span data-stu-id="c3b67-377">For now, the setting for `useMockData` is set to `true`.</span></span> <span data-ttu-id="c3b67-378">此设置将在一段时间内从配置中读取。</span><span class="sxs-lookup"><span data-stu-id="c3b67-378">This setting will be read from configuration in a moment.</span></span> <span data-ttu-id="c3b67-379">此时，应用会在运行时编译并成功加载，如图4-12 所示。</span><span class="sxs-lookup"><span data-stu-id="c3b67-379">At this point, the app compiles and should load successfully when run, as shown in Figure 4-12.</span></span>
...
<span data-ttu-id="c3b67-396">首先，修改端口上的应用 `appsettings.json` 文件，并在根中添加这两个设置：</span><span class="sxs-lookup"><span data-stu-id="c3b67-396">First, modify the ported app's `appsettings.json` file and add the two settings in the root:</span></span>
...
<span data-ttu-id="c3b67-407">应用的迁移即将完成。</span><span class="sxs-lookup"><span data-stu-id="c3b67-407">The app's migration is nearly complete.</span></span> <span data-ttu-id="c3b67-408">唯一剩余的任务是数据访问配置。</span><span class="sxs-lookup"><span data-stu-id="c3b67-408">The only remaining task is data access configuration.</span></span>
...
<span data-ttu-id="c3b67-418">创建时，必须将连接字符串传递到构造函数中 `DbContext` 。</span><span class="sxs-lookup"><span data-stu-id="c3b67-418">The connection string must be passed into the constructor when the `DbContext` is created.</span></span> <span data-ttu-id="c3b67-419">由于实例是由 Autofac 创建的，因此需要在中进行更改 `ApplicationModule` 。</span><span class="sxs-lookup"><span data-stu-id="c3b67-419">Since the instances are created by Autofac, the change needs to be made in `ApplicationModule`.</span></span> <span data-ttu-id="c3b67-420">修改模块以在 `connectionString` 其构造函数中采用，并将其分配给字段。</span><span class="sxs-lookup"><span data-stu-id="c3b67-420">Modify the module to take in a `connectionString` in its constructor and assign it to a field.</span></span> <span data-ttu-id="c3b67-421">然后修改的注册 `CatalogDBContext` ，将连接字符串作为参数添加：</span><span class="sxs-lookup"><span data-stu-id="c3b67-421">Then modify the registration for `CatalogDBContext` to add connection string as a parameter:</span></span>
...
<span data-ttu-id="c3b67-422">还必须将参数添加到新的构造函数重载 `CatalogDBContext` 本身中：</span><span class="sxs-lookup"><span data-stu-id="c3b67-422">The parameter must also be added to a new constructor overload in `CatalogDBContext` itself:</span></span>
...
<span data-ttu-id="c3b67-423">最后， `ConfigureServices` 必须从中读取连接字符串 `Config` ，并在 `ApplicationModule` 其实例化时将其传递到：</span><span class="sxs-lookup"><span data-stu-id="c3b67-423">Finally, `ConfigureServices` must read the connection string from `Config` and pass it into the `ApplicationModule` when it instantiates it:</span></span>
...
<span data-ttu-id="c3b67-424">使用此代码后，应用程序将像以前一样运行，在为时连接到 SQL Server 的 `UseMockData` 数据库 `false` 。</span><span class="sxs-lookup"><span data-stu-id="c3b67-424">With this code in place, the app runs as it did before, connecting to a SQL Server database when `UseMockData` is `false`.</span></span>

<span data-ttu-id="c3b67-425">此时，可以在生产环境中部署和运行该应用，转换为 ASP.NET Core 但仍在 .NET Framework 和 EF 6 上运行。</span><span class="sxs-lookup"><span data-stu-id="c3b67-425">The app can be deployed and run in production at this point, converted to ASP.NET Core but still running on .NET Framework and EF 6.</span></span> <span data-ttu-id="c3b67-426">如果需要，可以将应用迁移到 .NET Core 和 Entity Framework Core 上运行，这会带来前面几章所述的其他优势。</span><span class="sxs-lookup"><span data-stu-id="c3b67-426">If desired, the app can be migrated to run on .NET Core and Entity Framework Core, which will bring additional advantages described in earlier chapters.</span></span> <span data-ttu-id="c3b67-427">此文档特定于实体框架， [它将 EF Core 和 EF 6 进行比较](/ef/efcore-and-ef6/) ，并包含一个网格，其中显示了哪些库支持每个单个功能。</span><span class="sxs-lookup"><span data-stu-id="c3b67-427">Specific to Entity Framework, [this documentation compares EF Core and EF 6](/ef/efcore-and-ef6/) and includes a grid showing which library supports each of dozens of individual features.</span></span>

### <a name="migrate-to-entity-framework-core"></a><span data-ttu-id="c3b67-428">迁移到 Entity Framework Core</span><span class="sxs-lookup"><span data-stu-id="c3b67-428">Migrate to Entity Framework Core</span></span>

<span data-ttu-id="c3b67-429">假设决定迁移到 EF Core，则步骤可能相当简单，特别是在原始应用使用基于代码的模型方法时。</span><span class="sxs-lookup"><span data-stu-id="c3b67-429">Assuming a decision is made to migrate to EF Core, the steps can be fairly straightforward, especially if the original app used a code-based model approach.</span></span> <span data-ttu-id="c3b67-430">[准备从 EF 6 到 EF Core 的端口](/ef/efcore-and-ef6/porting/)时，请查看要使用 EF Core 的目标版本中的功能可用性。</span><span class="sxs-lookup"><span data-stu-id="c3b67-430">When [preparing to port from EF 6 to EF Core](/ef/efcore-and-ef6/porting/), review the availability of features in the destination version of EF Core you'll be using.</span></span> <span data-ttu-id="c3b67-431">查看有关[从和基于 EDMX 的模型的迁移以及](/ef/efcore-and-ef6/porting/port-edmx)[从基于代码的模型移植](/ef/efcore-and-ef6/porting/port-code)的文档。</span><span class="sxs-lookup"><span data-stu-id="c3b67-431">Review the documentation on [porting from and EDMX-based model](/ef/efcore-and-ef6/porting/port-edmx) versus [porting from a code-based model](/ef/efcore-and-ef6/porting/port-code).</span></span>

<span data-ttu-id="c3b67-432">若要升级到 EF Core 2.2，所涉及的基本步骤是添加适当的 NuGet 包 (s) 和更新命名空间。</span><span class="sxs-lookup"><span data-stu-id="c3b67-432">To upgrade to EF Core 2.2, the basic steps involved are to add the appropriate NuGet package(s) and update namespaces.</span></span> <span data-ttu-id="c3b67-433">然后调整如何将连接字符串传递到 `DbContext` 类型，以及如何将其连接到依赖关系注入。</span><span class="sxs-lookup"><span data-stu-id="c3b67-433">Then adjust how the connection string is passed to the `DbContext` type and how they're wired up for dependency injection.</span></span>

<span data-ttu-id="c3b67-434">EF Core 作为包引用添加到项目中：</span><span class="sxs-lookup"><span data-stu-id="c3b67-434">EF Core is added as a package reference to the project:</span></span>

```xml
<PackageReference Include="Microsoft.EntityFrameworkCore" Version="2.2.6" />
```

<span data-ttu-id="c3b67-435">移除了对 EF 6 的引用：</span><span class="sxs-lookup"><span data-stu-id="c3b67-435">The reference to EF 6 is removed:</span></span>

```xml
<PackageReference Include="EntityFramework" Version="6.2.0" />
```

<span data-ttu-id="c3b67-436">编译器将在和中报告 `CatalogDBContext` 错误 `CatalogDBInitializer` 。</span><span class="sxs-lookup"><span data-stu-id="c3b67-436">The compiler will report errors in `CatalogDBContext` and `CatalogDBInitializer`.</span></span> <span data-ttu-id="c3b67-437">`CatalogDbContext` 需要删除旧的命名空间，并将其替换为 `Microsoft.EntityFrameworkCore` 。</span><span class="sxs-lookup"><span data-stu-id="c3b67-437">`CatalogDbContext` needs to have the old namespaces removed and replaced with `Microsoft.EntityFrameworkCore`.</span></span> <span data-ttu-id="c3b67-438">可删除其构造函数。</span><span class="sxs-lookup"><span data-stu-id="c3b67-438">Its constructors can be removed.</span></span> <span data-ttu-id="c3b67-439">`DbModelBuilder` 应该替换为 `ModelBuilder` 。</span><span class="sxs-lookup"><span data-stu-id="c3b67-439">`DbModelBuilder` should be replaced with `ModelBuilder`.</span></span> <span data-ttu-id="c3b67-440">用于配置类型的帮助器方法将移动到实现的单独类中 `IEntityTypeConfiguration<T>` 。</span><span class="sxs-lookup"><span data-stu-id="c3b67-440">The helper methods for configuring types are moved to separate classes implementing `IEntityTypeConfiguration<T>`.</span></span> <span data-ttu-id="c3b67-441">然后， `CatalogDBContext` 类的 `OnModelCreating` 方法只是：</span><span class="sxs-lookup"><span data-stu-id="c3b67-441">Then the `CatalogDBContext` class's `OnModelCreating` method simply becomes:</span></span>
...
<span data-ttu-id="c3b67-442">涉及的其他更改包括：</span><span class="sxs-lookup"><span data-stu-id="c3b67-442">Other changes involved include:</span></span>

- <span data-ttu-id="c3b67-443">`HasDatabaseGeneratedOption(DatabaseGeneratedOption.None)` 替换为 `ValueGeneratedNever()`</span><span class="sxs-lookup"><span data-stu-id="c3b67-443">`HasDatabaseGeneratedOption(DatabaseGeneratedOption.None)` replaced with `ValueGeneratedNever()`</span></span>
- <span data-ttu-id="c3b67-444">`HasRequired<T>` 替换为 `HasOne<T>`</span><span class="sxs-lookup"><span data-stu-id="c3b67-444">`HasRequired<T>` replaced with `HasOne<T>`</span></span>
- <span data-ttu-id="c3b67-445">已安装 `Microsoft.EntityFrameworkCore.Relational` 包</span><span class="sxs-lookup"><span data-stu-id="c3b67-445">Installed `Microsoft.EntityFrameworkCore.Relational` package</span></span>
- <span data-ttu-id="c3b67-446">添加构造函数以 `CatalogDBContext` 采用 `DbContextOptions` 并将其传递到基构造函数</span><span class="sxs-lookup"><span data-stu-id="c3b67-446">Add a constructor to `CatalogDBContext` taking `DbContextOptions` and passing it to the base constructor</span></span>

<span data-ttu-id="c3b67-447">的示例配置类如下 `CatalogType` 所示：</span><span class="sxs-lookup"><span data-stu-id="c3b67-447">An example configuration class for `CatalogType` is shown here:</span></span>
...
<span data-ttu-id="c3b67-448">`CatalogDBInitializer`及其基类 `CreateDatabaseIfNotExists<T>` 与 EF Core 不兼容。</span><span class="sxs-lookup"><span data-stu-id="c3b67-448">The `CatalogDBInitializer` and its base class, `CreateDatabaseIfNotExists<T>`, are incompatible with EF Core.</span></span> <span data-ttu-id="c3b67-449">此类的目的是创建数据库并为数据库创建种子。</span><span class="sxs-lookup"><span data-stu-id="c3b67-449">The purpose of this class is to create and seed the database.</span></span> <span data-ttu-id="c3b67-450">使用 EF Core 将使用以下方法[创建并删除关联的 `DbContext` 数据库](/ef/core/managing-schemas/ensure-created)：</span><span class="sxs-lookup"><span data-stu-id="c3b67-450">Using EF Core will [create and drop the associated database for a `DbContext`](/ef/core/managing-schemas/ensure-created) using these methods:</span></span>
...
<span data-ttu-id="c3b67-451">可以通过手动脚本或作为类型配置的一部分来实现 EF Core 中的数据种子设定。</span><span class="sxs-lookup"><span data-stu-id="c3b67-451">Seeding data in EF Core can be done with manual scripts, or as part of the type configuration.</span></span> <span data-ttu-id="c3b67-452">除了其他实体属性，还可以使用在类中配置种子数据 `IEntityTypeConfiguration` `builder.HasData()` 。</span><span class="sxs-lookup"><span data-stu-id="c3b67-452">Along with other entity properties, seed data can be configured in `IEntityTypeConfiguration` classes by using `builder.HasData()`.</span></span> <span data-ttu-id="c3b67-453">原始应用已从 *安装* 目录中的 CSV 文件加载种子数据。</span><span class="sxs-lookup"><span data-stu-id="c3b67-453">The original app loaded seed data from CSV files in the *Setup* directory.</span></span> <span data-ttu-id="c3b67-454">假设只有少量项，则可以将这些数据记录作为实体配置的一部分添加。</span><span class="sxs-lookup"><span data-stu-id="c3b67-454">Given that there are only a handful of items, these data records can instead be added as part of the entity configuration.</span></span> <span data-ttu-id="c3b67-455">此方法非常适用于不经常更改的表中的查找数据。</span><span class="sxs-lookup"><span data-stu-id="c3b67-455">This approach works well for lookup data in tables that change infrequently.</span></span> <span data-ttu-id="c3b67-456">将以下方法添加到 `CatalogTypeConfig` 的 `Configure` 方法可确保在创建数据库时存在相关行：</span><span class="sxs-lookup"><span data-stu-id="c3b67-456">Adding the following to `CatalogTypeConfig`'s `Configure` method ensures the associated rows are present when the database is created:</span></span>
...
<span data-ttu-id="c3b67-457">初始应用包含一个 `PreconfiguredData` 类，该类包含和的数据 `CatalogBrand` `CatalogType` ，因此使用此方法时， `HasData` 调用将减少到：</span><span class="sxs-lookup"><span data-stu-id="c3b67-457">The initial app includes a `PreconfiguredData` class, which includes data for `CatalogBrand` and `CatalogType`, so using this method the `HasData` call reduces to:</span></span>
...
<span data-ttu-id="c3b67-458">`CatalogItem`还可以从中提取数据 `PreconfiguredData` ，假定关联的图像保留在源代码管理中，这是应用程序正常运行所需的最后一个表。</span><span class="sxs-lookup"><span data-stu-id="c3b67-458">The `CatalogItem` data can also be pulled from `PreconfiguredData`, and assuming the associated images are kept in source control, that is the last table needed for the app to function.</span></span> <span data-ttu-id="c3b67-459">`CatalogDBInitializer`类可以与任何对它的引用一起被移除。</span><span class="sxs-lookup"><span data-stu-id="c3b67-459">The `CatalogDBInitializer` class can be removed, along with any references to it.</span></span> <span data-ttu-id="c3b67-460">`CatalogItemHiLoGenerator`还将删除目录中的类和 SQL 文件，并在 `Infrastructure`) 中删除对这些文件的任何引用 (`CatalogService` `ApplicationModule` 。</span><span class="sxs-lookup"><span data-stu-id="c3b67-460">The `CatalogItemHiLoGenerator` class and the SQL files in the `Infrastructure` directory are also removed, along with any references to them (in `CatalogService`, `ApplicationModule`).</span></span>

<span data-ttu-id="c3b67-461">由于消除了的特殊密钥生成器类 `CatalogItem` ，此代码现在将从 `CatalogItemConfig` 以下内容中删除：</span><span class="sxs-lookup"><span data-stu-id="c3b67-461">With the elimination of the special key generator classes for `CatalogItem`, this code now is removed from `CatalogItemConfig`:</span></span>
...
<span data-ttu-id="c3b67-462">进行这些修改后，ASP.NET Core 应用程序将生成，但它尚不能用于 EF Core，但仍必须配置为依赖项注入。</span><span class="sxs-lookup"><span data-stu-id="c3b67-462">With these modifications, the ASP.NET Core app builds, but it doesn't yet work with EF Core, which must still be configured for dependency injection.</span></span> <span data-ttu-id="c3b67-463">在 EF Core 中，对其进行配置的最简单方法是在中 `ConfigureServices` ：</span><span class="sxs-lookup"><span data-stu-id="c3b67-463">With EF Core, the simplest way to configure it is in `ConfigureServices`:</span></span>
...
<span data-ttu-id="c3b67-464">Autofac 的最终版本 `ApplicationModule` 仅配置一种类型，具体取决于应用是否已配置为使用模拟数据：</span><span class="sxs-lookup"><span data-stu-id="c3b67-464">The final version of Autofac's `ApplicationModule` only configures one type, depending on whether the app is configured to use mock data:</span></span>
...
<span data-ttu-id="c3b67-465">端口应用将运行，但如果配置为使用非模拟数据，则不会显示任何数据。</span><span class="sxs-lookup"><span data-stu-id="c3b67-465">The ported app runs, but doesn't display any data if configured to use non-mock data.</span></span> <span data-ttu-id="c3b67-466">`HasData`仅在应用迁移时插入通过添加的种子数据。</span><span class="sxs-lookup"><span data-stu-id="c3b67-466">The seed data added through `HasData` is only inserted when migrations are applied.</span></span> <span data-ttu-id="c3b67-467">源应用未使用迁移，如果存在，则不会按原样迁移。</span><span class="sxs-lookup"><span data-stu-id="c3b67-467">The source app didn't use migrations, and if it had, they wouldn't migrate as-is.</span></span> <span data-ttu-id="c3b67-468">最好的方法是从新的迁移脚本开始。</span><span class="sxs-lookup"><span data-stu-id="c3b67-468">The best approach is to start with a new migration script.</span></span> <span data-ttu-id="c3b67-469">为此，请添加的包引用， `Microsoft.EntityFrameworkCore.Design` 并在项目根目录中打开一个终端窗口。</span><span class="sxs-lookup"><span data-stu-id="c3b67-469">To do this, add a package reference for `Microsoft.EntityFrameworkCore.Design` and open a terminal window in the project root.</span></span> <span data-ttu-id="c3b67-470">然后运行：</span><span class="sxs-lookup"><span data-stu-id="c3b67-470">Then run:</span></span>

```dotnetcli
dotnet ef migrations add Initial
```

<span data-ttu-id="c3b67-471">删除现有的 *eShopPorted* 数据库（如果存在），然后运行：</span><span class="sxs-lookup"><span data-stu-id="c3b67-471">Drop the existing *eShopPorted* database if it exists, then run:</span></span>

```dotnetcli
dotnet ef database update
```

<span data-ttu-id="c3b67-472">这会创建数据库并将其作为种子。</span><span class="sxs-lookup"><span data-stu-id="c3b67-472">This creates and seeds the database.</span></span> <span data-ttu-id="c3b67-473">现已准备好运行，其中包含几个较小的小更新可解决。</span><span class="sxs-lookup"><span data-stu-id="c3b67-473">It's now ready to run, with a few small updates left to address.</span></span>

## <a name="fix-all-todo-tasks"></a><span data-ttu-id="c3b67-474">修复所有 TODO 任务</span><span class="sxs-lookup"><span data-stu-id="c3b67-474">Fix all TODO tasks</span></span>

<span data-ttu-id="c3b67-475">此时运行移植应用会显示页面上不显示图片。</span><span class="sxs-lookup"><span data-stu-id="c3b67-475">Running the ported app at this point reveals that no pictures are shown on the page.</span></span> <span data-ttu-id="c3b67-476">这是因为 `PictureUri` `CatalogItem` 从不设置的属性。</span><span class="sxs-lookup"><span data-stu-id="c3b67-476">This is because the `PictureUri` property of `CatalogItem` is never set.</span></span> <span data-ttu-id="c3b67-477">查看 `TODO` 我们使用 Visual Studio **任务列表** 创建的项的列表，只保留处于中的项 `CatalogController` ，并提供 "从 wwwroot 引用 pic" 的注释。</span><span class="sxs-lookup"><span data-stu-id="c3b67-477">Looking at the list of `TODO` items we created using Visual Studio's **Task List**, the only one that remains is in `CatalogController`, with a note to "Reference pic from wwwroot."</span></span> <span data-ttu-id="c3b67-478">相关代码是：</span><span class="sxs-lookup"><span data-stu-id="c3b67-478">The code in question is:</span></span>
...
<span data-ttu-id="c3b67-479">最简单的解决方法是在站点的公共 *wwwroot/图片* 目录中引用公共映像文件。</span><span class="sxs-lookup"><span data-stu-id="c3b67-479">The simplest fix is to reference the public image files in the site's public *wwwroot/Pics* directory.</span></span> <span data-ttu-id="c3b67-480">可以通过将方法替换为以下代码来完成此任务：</span><span class="sxs-lookup"><span data-stu-id="c3b67-480">This task can be accomplished by replacing the method with the following code:</span></span>
...
<span data-ttu-id="c3b67-481">进行此更改后，运行应用程序将使图像像以前一样工作。</span><span class="sxs-lookup"><span data-stu-id="c3b67-481">With this change, running the app reveals the images work as before.</span></span>

## <a name="additional-mvc-customizations"></a><span data-ttu-id="c3b67-482">其他 MVC 自定义</span><span class="sxs-lookup"><span data-stu-id="c3b67-482">Additional MVC customizations</span></span>

<span data-ttu-id="c3b67-483">*EShopLegacyMVC* 应用程序非常简单，因此在默认 MVC 行为方面没有太多的配置。</span><span class="sxs-lookup"><span data-stu-id="c3b67-483">The *eShopLegacyMVC* app is fairly simple, so there isn't much to configure in terms of default MVC behavior.</span></span> <span data-ttu-id="c3b67-484">但是，如果确实需要配置其他 MVC 组件（如 CORS、筛选器和路由约束），则通常会在中提供此信息 `Startup.ConfigureServices` ，其中 `UseMvc` 调用。</span><span class="sxs-lookup"><span data-stu-id="c3b67-484">However, if you do need to configure additional MVC components, such as CORS, filters, and route constraints, you generally provide this information in `Startup.ConfigureServices`, where `UseMvc` is called.</span></span> <span data-ttu-id="c3b67-485">例如，下面的代码列表配置 [CORS](/aspnet/core/security/cors?preserve-view=true&view=aspnetcore-2.2) 并设置全局操作筛选器：</span><span class="sxs-lookup"><span data-stu-id="c3b67-485">For example, the following code listing configures [CORS](/aspnet/core/security/cors?preserve-view=true&view=aspnetcore-2.2) and sets up a global action filter:</span></span>
...
> [!Note]
> <span data-ttu-id="c3b67-486">若要完成 CORS 配置，还必须 `app.UseCors()` 在中调用 `Configure` 。</span><span class="sxs-lookup"><span data-stu-id="c3b67-486">To finish configuring CORS, you must also call `app.UseCors()` in `Configure`.</span></span>

<span data-ttu-id="c3b67-487">详细的 ASP.NET Core 文档中介绍了其他高级方案，如添加 [自定义模型联编](/aspnet/core/mvc/advanced/custom-model-binding?preserve-view=true&view=aspnetcore-2.2)程序、格式化程序等。通常，这些应用程序可以应用于单个控制器或操作基础上，也可以使用上述代码列表中所示的相同选项方法全局应用。</span><span class="sxs-lookup"><span data-stu-id="c3b67-487">Other advanced scenarios, like adding [custom model binders](/aspnet/core/mvc/advanced/custom-model-binding?preserve-view=true&view=aspnetcore-2.2), formatters, and more are covered in the detailed ASP.NET Core docs. Generally these can be applied on an individual controller or action basis, or globally using the same options approach shown in the previous code listing.</span></span>

## <a name="other-dependencies"></a><span data-ttu-id="c3b67-488">其他依赖项</span><span class="sxs-lookup"><span data-stu-id="c3b67-488">Other dependencies</span></span>

<span data-ttu-id="c3b67-489">使用依赖于旧配置模型的 .NET Framework 功能的依赖项（例如 WCF 客户端类型和跟踪代码）必须在移植时修改。</span><span class="sxs-lookup"><span data-stu-id="c3b67-489">Dependencies that use .NET Framework features that had a dependency on the legacy configuration model, such as the WCF client type and tracing code, must be modified when ported.</span></span> <span data-ttu-id="c3b67-490">它们不是直接请求配置信息，而是应在代码中进行配置。</span><span class="sxs-lookup"><span data-stu-id="c3b67-490">Rather than having these types pull in their configuration information directly, they should be configured in code.</span></span> <span data-ttu-id="c3b67-491">例如，与在 ASP.NET 应用程序的 *web.config* 中配置的 WCF 服务的连接，可以使用 `basicHttpBinding` 以下代码以编程方式进行配置：</span><span class="sxs-lookup"><span data-stu-id="c3b67-491">For example, a connection to a WCF service that was configured in an ASP.NET app's *web.config* to use `basicHttpBinding` could instead be configured programmatically with the following code:</span></span>
...
<span data-ttu-id="c3b67-492">WCF 客户端和其他 .NET Framework 类型应在代码中指定其设置，而不是依赖于配置文件的设置。</span><span class="sxs-lookup"><span data-stu-id="c3b67-492">Rather than relying on config files for its settings, WCF clients and other .NET Framework types should have their settings specified in code.</span></span> <span data-ttu-id="c3b67-493">通过这种方式进行配置，这些类型可在 ASP.NET Core 2.2 应用中继续工作。</span><span class="sxs-lookup"><span data-stu-id="c3b67-493">Configured in this manner, these types can continue to work in ASP.NET Core 2.2 apps.</span></span>

## <a name="references"></a><span data-ttu-id="c3b67-494">参考</span><span class="sxs-lookup"><span data-stu-id="c3b67-494">References</span></span>

- [<span data-ttu-id="c3b67-495">eShopModernizing GitHub 存储库</span><span class="sxs-lookup"><span data-stu-id="c3b67-495">eShopModernizing GitHub repository</span></span>](https://github.com/dotnet-architecture/eShopModernizing)
- [<span data-ttu-id="c3b67-496">.NET 升级助手工具</span><span class="sxs-lookup"><span data-stu-id="c3b67-496">.NET Upgrade Assistant tool</span></span>](https://aka.ms/dotnet-upgrade-assistant)
- [<span data-ttu-id="c3b67-497">API 和 Viewmodel 不应引用域模型</span><span class="sxs-lookup"><span data-stu-id="c3b67-497">Your API and ViewModels Should Not Reference Domain Models</span></span>](https://ardalis.com/your-api-and-view-models-should-not-reference-domain-models/)
- [<span data-ttu-id="c3b67-498">开发人员异常页中间件</span><span class="sxs-lookup"><span data-stu-id="c3b67-498">Developer Exception Page Middleware</span></span>](/aspnet/core/fundamentals/error-handling#developer-exception-page)
- [<span data-ttu-id="c3b67-499">深入了解 EF Core HasData</span><span class="sxs-lookup"><span data-stu-id="c3b67-499">Deep Dive into EF Core HasData</span></span>](/archive/msdn-magazine/2018/august/data-points-deep-dive-into-ef-core-hasdata-seeding)

>[!div class="step-by-step"]
><span data-ttu-id="c3b67-500">[上一页](more-migration-scenarios.md)
>[下一页](deployment-scenarios.md)</span><span class="sxs-lookup"><span data-stu-id="c3b67-500">[Previous](more-migration-scenarios.md)
